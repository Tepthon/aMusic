...
async def update_repo(_, message: Message):
    ...
    msg = await message.reply("🔄 `برمجه وكتابة محمد ...`")
    update_avail = updater()
    if update_avail:
        await msg.edit("👤 يمكن شراء بوت بحقوقك كامل \n\n• او نسخه سورس بحقوقك بأرخص الأسعار حسابي الوحيد : P17_12.")
        system("git pull -f && pip3 install -r requirements.txt")
        execle(sys.executable, sys.executable, "main.py", environ)
        return
    await msg.edit("My only **account on ** telegram [𝐷𝐸𝑉 𝑀𝑂𝐻𝐴𝑀𝑀𝐴𝐷 𝅘𝅥𝅯.](https://t.me/P17_12)", disable_web_page_preview=True)
